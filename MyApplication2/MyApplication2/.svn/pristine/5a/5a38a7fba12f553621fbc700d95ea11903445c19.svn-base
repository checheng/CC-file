...
public class UDPSocketClientManage {
	// 服务器IP
	private static String SERVER_IP = "59.172.153.70";
	// 服务器端口
	private static int LOCAL_PORT_AUDIO = 7000;
	// 接收数据包
...
//			msocketClient = new DatagramSocket(LOCAL_PORT_AUDIO);
			if(msocketClient==null){
				msocketClient = new DatagramSocket(null);
				msocketClient.setReuseAddress(true);
				msocketClient.bind(new InetSocketAddress(LOCAL_PORT_AUDIO));
			}
			// 接收数据缓存
			byte[] Buffer_Receive = new byte[2048];
			// 接收包
			Packet_Receive = new DatagramPacket(Buffer_Receive, 2048);
	...
	private class Thread_Send implements Runnable {
		// 发送数据缓存
		private byte[] Buffer_Send = new byte[2048];
		// 发送数据包
		...
		public Thread_Send(byte[] data, int len) {
			// 发送包
			Packet_Send = new DatagramPacket(Buffer_Send, len);
